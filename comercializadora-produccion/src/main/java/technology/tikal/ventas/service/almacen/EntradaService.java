 ...
 */
@RestController
@RequestMapping("/pedido/{pedidoId}/almacen/entrada")
public class EntradaService extends RestControllerTemplate {
...
private EntradaController entradaController;
    ...
    public void actualizar(@PathVariable final Long pedidoId, @PathVariable final Long entradaId,
        ...
        if (Long.compare(request.getId(), entradaId) != 0) {
            throw new MessageSourceResolvableException(new DefaultMessageSourceResolvable(
                new String[]{"NoValidRequest.EntradaService.actualizar"}, 
    ...
    public Page<Entrada[]> consultar(@PathVariable final Long pedidoId,
        ...
        Page<Entrada[]> r = PaginationModelFactory.getPage(
                entradaController.consultar(pedidoId, filter, pagination),
                "Entrada",
    ...
    @RequestMapping(produces = "application/json;charset=UTF-8", value="/{entradaId}", method = RequestMethod.GET)
    public Entrada get(@PathVariable final Long pedidoId, @PathVariable final Long entradaId) {
        return entradaController.get(pedidoId, entradaId);
    }
    
    @RequestMapping(value="/{entradaId}", method = RequestMethod.DELETE)
    @ResponseStatus(HttpStatus.ACCEPTED)
    public void borrar(@PathVariable final Long pedidoId, @PathVariable final Long entradaId) {
        entradaController.borrar(pedidoId, entradaId);
    }
